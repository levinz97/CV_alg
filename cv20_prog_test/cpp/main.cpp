    ...
    for (size_t y = 0; y < image_out.rows; y++){
        for (size_t x = 0; x < image_out.cols; x++){
            for(size_t c = 0; c < image_out.channels(); c++){
                auto intensity =  image_out.at<uchar>(y, x, c);
//            cout << intensity << endl;
                if (intensity + offset > 255){
                    image_out.at<uchar>(y,x,c)= 255;
                }
                else{
                    intensity + offset < 0 ?
                    image_out.at<uchar>(y,x,c) = 0 : image_out.at<uchar>(y,x,c) += offset;
//                    uchar tmp = image_out.at<uchar>(y,x,c);
//                    cout << (int)tmp << endl;
                }
            }
        }
